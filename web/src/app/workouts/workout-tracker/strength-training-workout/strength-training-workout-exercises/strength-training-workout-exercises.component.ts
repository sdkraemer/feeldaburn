...
export class StrengthTrainingWorkoutExercisesComponent implements OnInit {
  ...
  addExerciseControlsFromWorkout() {
    ...
    this.workout.exercises.forEach(exercise => {
      let exerciseGroup = this.strengthTrainingFormFactoryService.createExerciseFormGroup(
        exercise
      );
  ...
  private updatePercentage() {
    this.percentage =
      (this.activeExerciseIndex + 1) / this.workout.exercises.length * 100; //TODO: duplicated code
  }
