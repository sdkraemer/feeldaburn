...
export class WorkoutFactoryService {
    ...
    public createOrRetrieve(workoutId, workoutType, guideId): Observable<IWorkout> {
        let workout$;
        if(workoutId) {
    ...
    private retrieveExistingWorkout(workoutId): Observable<IWorkout> {
    ...
    private createNewWorkout(workoutType, guideId): Observable<IWorkout> {
        let workout$;
        if(guideId) {
            workout$ = this.retrieveStrengthTrainingWorkoutWithPreviousWorkouts(guideId);
        }
        else {
    ...
    private retrieveStrengthTrainingWorkoutWithPreviousWorkouts(guideId) {
        ...
        const previousWorkouts$ = this.workoutService.getPreviousStrengthTrainingWorkouts(guideId);
        let workout$ = Observable
            .zip(guide$, previousWorkouts$, (guide, previousWorkouts) => {
                let workout = <IStrengthTrainingWorkout>this.workoutFactory.createFromGuide(guide);
