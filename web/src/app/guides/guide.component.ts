...
export class GuideComponent implements OnInit {
  ...
  private getGuide() {
    if (this._id == "New") {
      ...
      this.guideService.getGuide(this._id).subscribe((guide: IGuide) => {
        this.form.setValue({
          _id: guide._id,
          name: guide.name,
  ...
  save() {
    ...
    if (guide._id) {
      this.guideService.update(guide).subscribe((isSuccessful: boolean) => {
        this.goToGuides();
      });
    } else {
      this.guideService.add(guide).subscribe((isSuccessful: boolean) => {
        this.goToGuides();
      });
    }
    this.goToGuides();
  ...
  moveExerciseUp(exercise, index) {
    ...
    <FormGroup>this.form.controls["exercises"]["controls"][
      index - 1
    ].patchValue({
      order: index - 1
    });
